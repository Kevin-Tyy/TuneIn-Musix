	...
	return (
		<div
			...
			{currentTrack.id === item.id && isPlaying ? (
				...
				<p className={ clsx('font-bold transition duration-500', 
					currentTrack.id == item.id && 'text-fuchsia-600'
				)}>{index! + 1}</p>
			)}
			<div
				onClick={handlePlayTrack}
				className={`flex justify-between w-full h-[90px] ${
				...
				<div className="flex gap-4">
					...
					<div className="flex flex-col space-y-1 justify-center">
						<h1
							className={`${
								currentTrack.id === item.id && "text-fuchsia-500 transition duration-500"
							}`}>
							{item?.name}
						</h1>
